...
分离线程通常被称为守护线程，这个名字来源于 UNIX 的守护进程概念——守护进程在后台运行，没有任何显式的用户界面。这类线程通常运行时间很长；它们可能会在应用程序的几乎整个生命周期中运行，执行一些后台任务，比如监控文件系统、清理对象缓存中未使用的条目，或者优化数据结构。另一方面，当有其他机制来识别线程何时完成，或者线程被用作“发出就忘”的任务时，使用分离线程也是合理的。
