...
一个特别极端（但非常常见）的情况是，共享数据只需要在初始化时防止并发访问，而初始化完成后就不再需要显式的同步。这可能是因为数据一旦创建就是只读的，因此不存在同步问题；也可能是因为必要的保护已经在操作数据的过程中隐式完成。不管是哪种情况，在数据初始化后，仅为了保护初始化而去锁定互斥锁都是多余的，会不必要地影响性能。正因为如此，C++ 标准提供了一种机制，专门用于在初始化期间保护共享数据。
